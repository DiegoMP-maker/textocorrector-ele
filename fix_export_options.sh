#!/bin/bash
# Script para reemplazar la función problemática en streamlit_app.py

set -e  # Detener en caso de error

echo "=== SOLUCIÓN DEFINITIVA PARA EL ERROR ST.BUTTON() EN ST.FORM() ==="
echo "Este script reemplazará la implementación problemática con una versión optimizada."
...
BACKUP_FILE="streamlit_app.py.bak.$(date +%Y%m%d%H%M%S)"
...
def ui_export_options_rediseñado(data):
    """
    Implementación totalmente rediseñada sin formularios ni botones anidados.
    Esta versión utiliza un enfoque de selección por checkboxes y un solo botón
    para procesar todas las opciones a la vez.
    ...
    instance_key = hash(str(data)[:100])
    ...
    with formato_cols[0]:
        word_selected = st.checkbox(
            "Documento Word (DOCX)", 
            value=st.session_state[f"select_docx_{instance_key}"],
            key=f"select_docx_{instance_key}"
        )
    with formato_cols[1]:
        html_selected = st.checkbox(
            "Página web (HTML)", 
            value=st.session_state[f"select_html_{instance_key}"],
            key=f"select_html_{instance_key}"
        )
    with formato_cols[2]:
        csv_selected = st.checkbox(
            "Datos CSV", 
            value=st.session_state[f"select_csv_{instance_key}"],
            key=f"select_csv_{instance_key}"
        )
    
    # IMPORTANTE: Almacenar el estado actual
    st.session_state[f"select_docx_{instance_key}"] = word_selected
    st.session_state[f"select_html_{instance_key}"] = html_selected
    st.session_state[f"select_csv_{instance_key}"] = csv_selected
    ...
    # Forzar rerun si el estado de selección cambió
    formato_changed = False
    for fmt in ["docx", "html", "csv"]:
        if f"prev_{fmt}_{instance_key}" in st.session_state:
            if st.session_state[f"prev_{fmt}_{instance_key}"] != st.session_state[f"select_{fmt}_{instance_key}"]:
                formato_changed = True
    
    # Guardar estado anterior
    for fmt in ["docx", "html", "csv"]:
        st.session_state[f"prev_{fmt}_{instance_key}"] = st.session_state[f"select_{fmt}_{instance_key}"]
    
    # PASO 2: Botón para preparar informes
    if st.button("Preparar informes seleccionados", key=f"prepare_exports_{instance_key}"):
        if not (word_selected or html_selected or csv_selected):
            ...
            # Para evitar que el botón desaparezca al hacer clic
            st.rerun()
    
    # PASO 3: Mostrar resultados si se activó la bandera
