                ...
                with st.expander(f"{ejercicio.get('titulo', f'Ejercicio {i+1}')}"):
                    ...
                    with solucion_tab:
                        ...
                        try:
                            ...
                            st.success(
                                f"✅ Estadísticas guardadas en hoja de Seguimiento.")
                        except NameError:
                            # Si tracking_sheet no está definido, intentamos recuperarlo
                            tracking_doc = client_gsheets.open_by_key(
                                TRACKING_DOC_ID)
                            try:
                                tracking_sheet = tracking_doc.worksheet(
                                    "Seguimiento")
                            except gspread.exceptions.WorksheetNotFound:
                                tracking_sheet = tracking_doc.add_worksheet(
                                    title="Seguimiento", rows=100, cols=14)
...
with st.spinner("Generando audio con ElevenLabs..."):
    ...
    try:
        ...
        if response_audio.ok:
            ...
            st.warning(
                f"⚠️ No se pudo reproducir el consejo con ElevenLabs. (Status code: {response_audio.status_code})")
    except Exception as e:
...
try:
    # Aquí debería ir el código del bloque try
    pass
except Exception as e:
...
"""
    ...
    # Aquí puedes agregar más código para manejar el error específico
                    ...
                    try:
                        ...
                        st.error(
                            f"Error al preparar los datos de seguimiento: {e}")
